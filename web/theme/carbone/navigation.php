...
function get_menu_global($navigation) {
    ...
    if(isset($tmp)) {
        $data.= '<div class="collapse navbar-collapse" id="navbar-responsive">';
        ...
        foreach($tmp as $v) {
            if(!isset($v['rubrique'])) { // rubrique simple 
                if($v['url']!='') {
                    ...
                    if($v['libelle']!='')
                        $data.="<li class=\"dropdown-header\">".$v['libelle']."</li>\n";
                    else
...
function get_menu_local($navigation, $get) {
    ...
    foreach($navigation as $v) {
        ...
        if(isset($v['image']))
            $v['image'] = CFG_PATH_HTTP_WEB.'/theme/'.$cfg_profil['theme'].'/dist/images/'.$v['image'];
        else
            $v['image'] = '';
